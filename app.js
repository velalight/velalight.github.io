...
"نساعدك تختار الرائحة والتفاصيل المناسبة.",
   ...
   cart_items_title: "منتجاتك",
   cart_coupon_title: "🎟️ كوبون الخصم",
   cart_coupon_placeholder: "أدخل كود الكوبون",
   cart_coupon_apply: "تطبيق",
   cart_delivery_title: "📝 بيانات التوصيل",
   cart_name_ph: "الاسم بالكامل",
   cart_phone_ph: "رقم الموبايل",
   cart_email_ph: "البريد الإلكتروني (اختياري)",
   cart_city_ph: "المحافظة",
   cart_address_ph: "العنوان بالتفصيل",
   cart_notes_ph: "ملاحظات (اختياري)",
   cart_payment_title: "💳 طريقة الدفع",
   cart_payment_desc: "سيتم إرسال تفاصيل الدفع (InstaPay / فودافون كاش / أورنج كاش / تحويل بنكي) عبر الواتساب فور تأكيد الطلب. الشحن يُدفع كاش عند الاستلام.",
   cart_subtotal: "المجموع الفرعي",
   cart_discount: "الخصم",
   cart_total: "الإجمالي",
   cart_free_shipping_progress: "أضف {amount} للحصول على شحن مجاني! 🚚",
   cart_free_shipping_earned: " مبروك! طلبك مؤهل لشحن مجاني",
   cart_qty_discount: "🎁 خصم الكمية (3+ قطع)",
   cart_coupon_discount: "🎟️ خصم الكوبون",
   cart_checkout_btn: "✅ إتمام الطلب عبر واتساب",
   cart_empty_cart_btn: "🗑️ إفراغ السلة",
   cart_confirm_empty: "هل أنت متأكد من إفراغ السلة؟",
   cart_trust_secure: "دفع آمن",
   cart_trust_refund: "ضمان استرجاع",
   cart_trust_delivery: "توصيل موثوق",
   cart_scent_required: "اختر العطر",
   cart_item_total: "إجمالي الصنف:",
   cart_add_to_cart: "أضف للسلة",
   cart_out_of_stock: "نفدت الكمية",
   cart_cross_sell_title: "أرشح لك فوحات شمع معطره لدولابك",
   cart_cross_sell_btn: "أضف للسلة",
 },
 /* ═══════════════════════════════════════
    🇬🇧 ENGLISH
    ═══════════════════════════════════════ */
...
"We help you choose the right scent and details for your taste.",
  /* ═══ FAQ ═══ */
   ...
   cart_coupon_title: "🎟️ Coupon Code",
   cart_coupon_placeholder: "Enter coupon code",
   cart_coupon_apply: "Apply",
   cart_delivery_title: "📝 Delivery Details",
   cart_name_ph: "Full Name",
   cart_phone_ph: "Mobile Number",
   cart_email_ph: "Email (Optional)",
   cart_city_ph: "Governorate",
   cart_address_ph: "Full Address",
   cart_notes_ph: "Notes (Optional)",
   cart_payment_title: "💳 Payment Method",
   cart_payment_desc: "Payment details (InstaPay / Vodafone Cash / Orange Cash / Bank Transfer) will be sent via WhatsApp upon order confirmation. Shipping is paid cash on delivery.",
   cart_subtotal: "Subtotal",
   cart_discount: "Discount",
   cart_total: "Total",
   cart_free_shipping_progress: "Add {amount} more for FREE shipping! 🚚",
   cart_free_shipping_earned: "🎉 Congrats! You qualify for FREE shipping",
   cart_qty_discount: "🎁 Quantity Discount (3+ items)",
   cart_coupon_discount: "🎟️ Coupon Discount",
   cart_checkout_btn: "✅ Complete Order via WhatsApp",
   cart_empty_cart_btn: "🗑️ Empty Cart",
   cart_confirm_empty: "Are you sure you want to empty the cart?",
   cart_trust_secure: "Secure Payment",
   cart_trust_refund: "Refund Guarantee",
   cart_trust_delivery: "Reliable Delivery",
   cart_scent_required: "Choose scent",
   cart_item_total: "Item total:",
   cart_add_to_cart: "Add to Cart",
   cart_out_of_stock: "Out of stock",
   cart_cross_sell_title: "I recommend scented wax fresheners for your closet",
   cart_cross_sell_btn: "Add to Cart",
 }
};
...
article.innerHTML = `<a class="p-media" href="product.html?p=${p.id}" aria-label="${pname(p)}">${mediaContent}${p.pinned ? `<span class="p-pin-badge">📌 مميز</span>` : ""}${badge ? `<span class="p-badge">${badge}</span>` : ""}${stockBadg}<span class="p-quick">${t("view_details")}</span></a><div class="p-body"><span class="p-cat">${cat(p.cat)}</span><h3><a href="product.html?p=${p.id}">${pname(p)}</a></h3>${r ? `<span class="stars" aria-label="${Math.round(r.avg)} stars">${"★".repeat(Math.round(r.avg))}</span>` : ""}<p class="p-desc">${productDesc}</p>${productDesc.length > 30 ? `<a href="product.html?p=${p.id}" class="p-desc-link">${readMoreText}</a>` : ""}<div class="p-foot"><div class="p-price"><span class="current-price">${money(p.price)}</span>${p.old > p.price ? `<del>${money(p.old)}</del>` : ""}</div><div style="display:flex; gap:6px; width:100%; margin-top:4px;"><button class="p-add" data-id="${p.id}" ${isOutOfStock ? "disabled" : ""} style="flex:1;">${isOutOfStock ? (LANG === "en" ? "Out of stock" : "نفدت الكمية") : t("add_cart")}</button><button class="p-wish" data-wish="${p.id}" type="button" aria-label="أضف للمفضلة" style="background:${inWishlist ? '#fee' : 'var(--bg)'}; border:1px solid ${inWishlist ? '#e74c3c' : 'var(--line)'}; border-radius:10px; cursor:pointer; font-size:1.1rem; transition:.2s; color:${inWishlist ? '#e74c3c' : 'inherit'}; display:flex; align-items:center; justify-content:center; width:42px; height:42px; padding:0; flex-shrink:0;">${inWishlist ? "❤️" : "🤍"}</button></div></div></div>`;
...
if (!reviews.length) {
grid.innerHTML = `<div class="empty" style="padding:3rem 0;"><div style="font-size:3rem;margin-bottom:1rem;"></div><p style="color:var(--mut);">${LANG === "en" ? "No reviews yet. Be the first!" : "لسة مفيش مراجعات. كن أول من يشارك رأيه!"}</p></div>`;
return;
}
const frag = document.createDocumentFragment();
reviews.forEach((r) => {
...
w.innerHTML = `<div class="vl-quick-preview" style="display:flex;align-items:center;gap:.9rem;margin-bottom:1rem;padding:.65rem;border:1px solid var(--line);border-radius:14px;background:var(--bg);"><img src="${imgSrc}" alt="${safeName}" width="72" height="72" loading="eager" decoding="async" style="width:72px;height:72px;object-fit:cover;border-radius:11px;flex:0 0 72px;" onerror="this.style.display='none'"><div style="min-width:0;flex:1;"><strong style="display:block;font-size:.98rem;line-height:1.35;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${safeName}</strong><span style="display:block;margin-top:.25rem;font-weight:800;color:var(--gold2);">${priceText}</span>${quickAddMaxStock < 99 ? `<small style="display:block;margin-top:.2rem;color:var(--mut);">${LANG === "en" ? `${quickAddMaxStock} available` : `متاح ${quickAddMaxStock} فقط`}</small>` : ""}</div></div><label for="modalScentSelect" style="display:block;font-weight:700;margin-bottom:.45rem;">${t("quick_add_scent")}</label><select id="modalScentSelect" aria-required="true" style="width:100%;padding:.85rem;border:1px solid var(--line);border-radius:11px;background:var(--bg);color:var(--dark);font-family:inherit;font-size:1rem;cursor:pointer;outline:none;"><option value="">${LANG === "en" ? "Choose a scent..." : "اختر العطر..."}</option>${VELA_SCENTS.map(s => `<option value="${s[0]}">${velaScentTr(s[0])}</option>`).join('')}</select><div style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;margin-top:1rem;"><span style="font-weight:700;">${t("quick_add_qty")}</span><div style="display:flex;align-items:center;gap:.65rem;border:1px solid var(--line);border-radius:11px;padding:.25rem;background:var(--bg);"><button id="smQMinus" type="button" aria-label="${LANG === "en" ? "Decrease quantity" : "تقليل الكمية"}" style="width:36px;height:36px;border:0;border-radius:8px;background:transparent;font-size:1.25rem;cursor:pointer;">−</button><b id="smQVal" style="min-width:22px;text-align:center;">1</b><button id="smQPlus" type="button" aria-label="${LANG === "en" ? "Increase quantity" : "زيادة الكمية"}" style="width:36px;height:36px;border:0;border-radius:8px;background:transparent;font-size:1.25rem;cursor:pointer;">+</button></div></div>`;
updateQuickAddQtyUI();
openDrawer("scentOv");
setTimeout(() => {
...
div.className = 'scent';
div.innerHTML = `<i>${i+1}</i><div><b>${LANG==="en"?s[1]:s[0]}</b><small>${LANG==="en"?s[0]:s[1]}</small></div>`;
...
/* ═══════════════════════════════════════════════════════════
✨ [NEW] CART EXPERIENCE — تجربة سلة محسّنة (رحلة واحدة)
═══════════════════════════════════════════════════════════ */
function initCart(){
cartBadge();
fillCitySelect(document.getElementById("coCity"));
...
if(!confirm(t("cart_confirm_empty")||t("t_confirm_empty")))return;
saveCart([]);
renderCart();
cartBadge();
});
...
/* ═══════════════════════════════════════════════════════════
✨ [NEW] renderCart — عرض كل شيء في صفحة واحدة (رحلة واحدة)
═══════════════════════════════════════════════════════════ */
function renderCart(){
const c=getCart();
...
<div style="text-align:center; padding:3rem 1rem;">
  ...
  <h3 style="margin:0 0 0.5rem; color:var(--dark);">${t("cart_empty")}</h3>
  <p style="color:var(--mut); margin:0;">${t("cart_empty_sub")}</p>
</div>`;
updateTotals(c);
return;
}
// بناء HTML كامل للسلة — كل شيء في صفحة واحدة
let html = '';
// 1. قسم المنتجات
html += `
<div class="cart-section" style="margin-bottom:1.5rem;">
  <h3 style="margin:0 0 1rem; font-size:1.1rem; color:var(--dark); display:flex; align-items:center; gap:0.5rem;">
    <span>🛍️</span>
    <span>${t("cart_items_title")||"منتجاتك"}</span>
    <span style="margin-inline-start:auto; background:var(--c-warm); color:var(--gold2); padding:0.2rem 0.7rem; border-radius:99px; font-size:0.8rem; font-weight:700;">${c.length}</span>
  </h3>
...
html += `
    <div class="citem" style="background:var(--panel); border:1px solid var(--line); border-radius:14px; padding:1rem; margin-bottom:0.8rem; display:flex; gap:1rem; align-items:flex-start; position:relative;">
      <button class="rm" type="button" data-i="${i}" aria-label="remove" style="position:absolute; top:0.5rem; inset-inline-end:0.5rem; background:transparent; border:0; color:var(--mut); cursor:pointer; font-size:1.2rem; padding:0.3rem; line-height:1;">✕</button>
      <div class="citem-media" style="flex:0 0 80px;">
        <img src="${it.img||''}" alt="${pname({name:it.name,nameEn:it.nameEn})}" loading="lazy" width="80" height="80" style="width:80px; height:80px; object-fit:cover; border-radius:10px;" onerror="window.handleImageError(this, '${it.id}')">
      </div>
      <div class="citem-info" style="flex:1; min-width:0;">
        <h5 style="margin:0 0 0.5rem; font-size:0.95rem; color:var(--dark);">${pname({name:it.name,nameEn:it.nameEn})}</h5>
        <label class="cart-scent-picker" style="display:block; margin-bottom:0.5rem;">
          <span class="cart-scent-label" style="font-size:0.8rem; color:var(--mut); display:block; margin-bottom:0.3rem;">${t("scent_lbl")}</span>
          <select class="cart-scent-select" data-i="${i}" aria-label="${t("scent_lbl")}" style="width:100%; padding:0.5rem; border:1px solid var(--line); border-radius:8px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.9rem;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-top:0.5rem;">
          <div style="font-size:0.85rem; color:var(--mut);">
            <span>${t("price_lbl")}</span>
            <strong style="color:var(--dark);">${money(it.price)}</strong>
          </div>
          <div class="qty" style="display:flex; align-items:center; gap:0.5rem; border:1px solid var(--line); border-radius:8px; padding:0.2rem;">
            <button class="cq-minus" type="button" data-i="${i}" aria-label="minus" style="width:28px; height:28px; border:0; background:transparent; cursor:pointer; font-size:1rem;">−</button>
            <b style="min-width:24px; text-align:center;">${it.qty}</b>
            <button class="cq-plus" type="button" data-i="${i}" aria-label="plus" style="width:28px; height:28px; border:0; background:transparent; cursor:pointer; font-size:1rem;">+</button>
          </div>
        </div>
        <div style="margin-top:0.5rem; padding-top:0.5rem; border-top:1px dashed var(--line); font-size:0.85rem; color:var(--gold2); font-weight:700;">
          ${t("cart_item_total")} <strong>${money(lineTotal)}</strong>
        </div>
      </div>
    </div>
`;
...
html += `
<div class="cart-section" style="margin-bottom:1.5rem;">
  <h3 style="margin:0 0 0.8rem; font-size:1rem; color:var(--dark); display:flex; align-items:center; gap:0.5rem;">
    <span>🎟️</span>
    <span>${t("cart_coupon_title")||"كوبون الخصم"}</span>
  </h3>
  <div style="display:flex; gap:0.5rem;">
    <input type="text" id="couponInput" placeholder="${t("cart_coupon_placeholder")||"أدخل كود الكوبون"}" style="flex:1; padding:0.7rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
    <button id="applyCouponBtn" type="button" style="padding:0.7rem 1.2rem; background:var(--c-warm); color:var(--gold2); border:1px solid var(--c-gold); border-radius:10px; cursor:pointer; font-weight:700; font-family:inherit; white-space:nowrap;">${t("cart_coupon_apply")||"تطبيق"}</button>
  </div>
  ${appliedCoupon ? `<div style="margin-top:0.5rem; font-size:0.85rem; color:var(--ok);">✓ ${appliedCoupon.code} ${LANG==="en"?"applied":"مطبق"}</div>` : ''}
</div>
`;
// 3. بيانات التوصيل
html += `
<div class="cart-section" style="margin-bottom:1.5rem;">
  <h3 style="margin:0 0 0.8rem; font-size:1rem; color:var(--dark); display:flex; align-items:center; gap:0.5rem;">
    <span>📝</span>
    <span>${t("cart_delivery_title")||"بيانات التوصيل"}</span>
  </h3>
  <div class="co-form" style="display:flex; flex-direction:column; gap:0.7rem;">
    <input type="text" id="coName" placeholder="${t("cart_name_ph")||"الاسم بالكامل"}" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
    <input type="tel" id="coPhone" placeholder="${t("cart_phone_ph")||"رقم الموبايل"}" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
    <input type="email" id="coEmail" placeholder="${t("cart_email_ph")||"البريد الإلكتروني (اختياري)"}" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
    <select id="coCity" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
      <option value="">${t("cart_city_ph")||"المحافظة"}</option>
    </select>
    <input type="text" id="coAddr" placeholder="${t("cart_address_ph")||"العنوان بالتفصيل"}" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none;">
    <textarea id="coNotes" placeholder="${t("cart_notes_ph")||"ملاحظات (اختياري)"}" rows="2" style="padding:0.8rem; border:1px solid var(--line); border-radius:10px; background:var(--bg); color:var(--dark); font-family:inherit; font-size:0.95rem; outline:none; resize:vertical;"></textarea>
  </div>
</div>
`;
// 4. طريقة الدفع
html += `
<div class="cart-section" style="margin-bottom:1rem;">
  <h3 style="margin:0 0 0.5rem; font-size:1rem; color:var(--dark); display:flex; align-items:center; gap:0.5rem;">
    <span>💳</span>
    <span>${t("cart_payment_title")||"طريقة الدفع"}</span>
  </h3>
  <div style="background:var(--c-warm); border:1px solid var(--c-gold); border-radius:10px; padding:0.8rem; font-size:0.85rem; color:var(--mut); line-height:1.6;">
    ${t("cart_payment_desc")||"سيتم إرسال تفاصيل الدفع (InstaPay / فودافون كاش / أورنج كاش / تحويل بنكي) عبر الواتساب فور تأكيد الطلب. الشحن يُدفع كاش عند الاستلام."}
  </div>
</div>
`;
w.innerHTML = html;
...
// تحديث الإجماليات
updateTotals(c);
// Cross-sell (اقتراح منتج)
renderCrossSell(c, w);
}
 /* ═══════════════════════════════════════════════════════════
✨ [NEW] renderCrossSell — اقتراح منتج إضافي
═══════════════════════════════════════════════════════════ */
function renderCrossSell(c, w) {
const products = (typeof ALL_PRODUCTS !== "undefined") ? ALL_PRODUCTS : [];
...
<div style="display:flex; align-items:center; gap:1rem;">
  ...
  <div style="flex:1">
    <div style="font-weight:700; font-size:0.9rem;">${t("cart_cross_sell_title")||"أرشح لك فوحات شمع معطره لدولابك"}: ${pname(suggestedProduct)}</div>
    <div style="font-size:0.8rem; color:var(--mut); margin:0.2rem 0;">${money(suggestedProduct.price)} ${LANG==="en"?"only":"فقط"}</div>
    <button class="btn sm" id="addSuggestBtn" style="margin-top:0.3rem; width:100%; padding:0.5rem; background:var(--gold2); color:#fff; border:0; border-radius:8px; cursor:pointer; font-weight:700;">${t("cart_cross_sell_btn")||"أضف للسلة"}</button>
  </div>
</div>
`;
...
/* ═══════════════════════════════════════════════════════════
✨ [NEW] updateTotals — حساب الإجماليات مع Sticky Footer
═══════════════════════════════════════════════════════════ */
...
// ===== تحديث الـ Sticky Footer =====
const dfoot = document.querySelector("#cartDrawer .dfoot");
if (dfoot) {
// بناء HTML جديد للـ footer
let footerHtml = '';
// ملخص الخصومات
if (finalDiscount > 0) {
footerHtml += `<div class="trow" style="display:flex; justify-content:space-between; padding:0.3rem 0; color:var(--ok); font-size:0.9rem;">
  <span>${appliedType === "qty" ? (t("cart_qty_discount")||"خصم الكمية") : (t("cart_coupon_discount")||"خصم الكوبون")}</span>
  <b>-${money(finalDiscount)}</b>
</div>`;
}
// الشحن المجاني
...
footerHtml += `<div class="trow" style="display:flex; justify-content:center; padding:0.5rem; background:#eafaf1; border-radius:8px; margin:0.5rem 0; color:#27ae60; font-weight:700; font-size:0.9rem;">
  <span>${t("cart_free_shipping_earned")||"🎉 مبروك! طلبك مؤهل لشحن مجاني"}</span>
</div>`;
} else {
footerHtml += `<div class="trow" style="padding:0.6rem; background:#fdf5ed; border-radius:8px; margin:0.5rem 0; border:1px solid var(--c-gold);">
  <div style="text-align:center; margin-bottom:0.4rem; font-size:0.85rem; color:var(--dark);">
    ${(t("cart_free_shipping_progress")||"أضف {amount} للحصول على شحن مجاني! ").replace('{amount}', money(remaining))}
  </div>
  <div style="background:#e0d6c8; height:6px; border-radius:3px; overflow:hidden;">
    <div style="background:linear-gradient(90deg, #d4af37, #f9d877); height:100%; width:${progressPercent}%; transition:width 0.5s ease-in-out; border-radius:3px;"></div>
  </div>
</div>`;
}
// الإجمالي
footerHtml += `<div class="trow total" style="display:flex; justify-content:space-between; align-items:center; padding:0.8rem 0; border-top:2px solid var(--line); margin-top:0.5rem;">
  <span style="font-size:1.1rem; font-weight:700; color:var(--dark);">${t("cart_total")||"الإجمالي"}</span>
  <b id="cartTotal" style="font-size:1.3rem; color:var(--gold2);">${money(total)}</b>
</div>`;
// Trust badges
footerHtml += `<div style="display:flex; justify-content:center; gap:1rem; margin:0.5rem 0; font-size:0.75rem; color:var(--mut); flex-wrap:wrap;">
  <span style="display:flex; align-items:center; gap:4px;">🔒 ${t("cart_trust_secure")||"دفع آمن"}</span>
  <span style="display:flex; align-items:center; gap:4px;">🔄 ${t("cart_trust_refund")||"ضمان استرجاع"}</span>
  <span style="display:flex; align-items:center; gap:4px;">🚚 ${t("cart_trust_delivery")||"توصيل موثوق"}</span>
</div>`;
// الأزرار
footerHtml += `<button id="checkoutBtn" type="button" style="width:100%; padding:1rem; background:#25D366; color:#fff; border:0; border-radius:12px; cursor:pointer; font-weight:700; font-size:1rem; font-family:inherit; margin-bottom:0.5rem; display:flex; align-items:center; justify-content:center; gap:0.5rem;">
  <span>✅</span>
  <span>${t("cart_checkout_btn")||"إتمام الطلب عبر واتساب"}</span>
</button>`;
footerHtml += `<button id="emptyCartBtn" type="button" style="width:100%; padding:0.7rem; background:transparent; color:var(--mut); border:1px solid var(--line); border-radius:10px; cursor:pointer; font-size:0.9rem; font-family:inherit;">
  🗑️ ${t("cart_empty_cart_btn")||"إفراغ السلة"}
</button>`;
dfoot.innerHTML = footerHtml;
// ربط الأحداث الجديدة
document.getElementById("checkoutBtn")?.addEventListener("click", checkout);
document.getElementById("emptyCartBtn")?.addEventListener("click", () => {
if(!confirm(t("cart_confirm_empty")||t("t_confirm_empty")))return;
...
w.innerHTML=res.map(p=>`<div class="sr-item" data-id="${p.id}"><img src="${imgOf(p)}" alt="" loading="lazy" width="50" height="50" onerror="window.handleImageError(this, '${p.id}')"><div><b>${pname(p)}</b><br><small>${money(p.price)}</small></div></div>`).join("");
