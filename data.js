const CFG={
  ...
  FIREBASE:{
    apiKey:"AIzaSyDTUq5rVccv2oLvpGYYZXHiteGuiE8y8o",
...
const CDN=u=>{
  if(!u)return"";
  if(u.startsWith("data:")||u.includes("cdn.jsdelivr.net")||u.startsWith("http"))
    return u.startsWith("http")?u.replace("https://velalight.github.io/",`https://cdn.jsdelivr.net/gh/${CFG.REPO}/`):u;
...
/* =========================================================
   VELALIGHT OFFICIAL SCENTS
   كل منتجات الموقع تستخدم هذه القائمة نفسها
   ========================================================= */
...
/* =========================================================
   PRODUCTS
   كل منتج متاح له كل العطور الرسمية الـ23
   ========================================================= */
...
const LocalDB={
  ...
  async update(c,id,d){
    this.s(c,this.g(c).map(x=>x.id===id?{...x,...d}:x))
  },

  async remove(c,id){
    this.s(c,this.g(c).filter(x=>x.id!==id))
  }
};
...
window.addEventListener("fb-ready",()=>{
  ...
  const tg=$("#dbModeTag");
  tg&&(tg.textContent="☁️ Firestore متصل");
...
const I18N={
  ...
  en:{
    ...
    scents_sub:"All 23 scents available",
