...
const money = (n) =>
  Number(n || 0).toLocaleString("en-US") + " " + (LANG === "en" ? "EGP" : "ج.م");
...
const CDN = (u) => {
  ...
  if (u.includes("cdn.jsdelivr.net")) return u;
  ...
  return `https://cdn.jsdelivr.net/gh/${repo}/${u.replace(/^\/+/, "")}`;
};
...
const SCENTS = [
  ...
  ["لافندر", "Lavender"],
  ["مسك أبيض", "White Musk"],
  ["ساندال وود", "Sandalwood"],
  ["باتشولي", "Patchouli"]
];

const scentTr = (n) => {
  const f = SCENTS.find((s) => s[0] === n || s[1] === n);
  return f ? (LANG === "en" ? f[1] : f[0]) : n;
};
...
const PRODUCTS = [
  {
    ...
    scents: ["عود", "فانيليا", "مسك أبيض"],
    img: "RRRR.jpg",
    imgs: ["RRRR.jpg", "RRRRR.jpg", "RRRRRR.jpg", "RR.jpg", "RRR.jpg"],
    sold: 0,
    createdAt: NOW - 1 * D,
  ...
  },
  {
    ...
    scents: ["ورد", "فانيليا", "مسك أبيض"],
    img: "heart2.jpg",
    imgs: ["heart2.jpg", "heart3.jpg", "heart1.jpg"],
    sold: 0,
    createdAt: NOW - 2 * D,
  ...
  },
  {
    ...
    scents: ["عنبر", "عود", "كراميل"],
    img: "candle1.jpg",
    imgs: ["candle1.jpg", "candle2.jpg", "candle3.jpg", "candle4.jpg", "candle5.jpg"],
    sold: 0,
    createdAt: NOW - 3 * D,
  ...
  },
  {
    ...
    hours: "72 ساعة اشتعال",
    hoursEn: "72h burn time",
    scents: ["قهوة", "موكا", "فانيليا"],
    img: "iccoffe1.jpg",
    imgs: ["iccoffe1.jpg", "iccoffe2.jpg", "iccoffe3.jpg", "iccoffe4.jpg"],
    sold: 0,
    createdAt: NOW - 4 * D,
  ...
  },
  {
    ...
    hours: "72 ساعة اشتعال",
    hoursEn: "72h burn time",
    scents: ["لافندر", "ياسمين", "كوكونات"],
    img: "gift1.jpg",
    imgs: ["gift1.jpg", "gift2.jpg", "gift3.jpg", "gift4.jpg", "gift5.jpg", "gift6.jpg", "gift7.jpg"],
    sold: 0,
    createdAt: NOW - 6 * D,
  ...
  },
  {
    ...
    hours: "72 ساعة اشتعال",
    hoursEn: "72h burn time",
    scents: ["لافندر", "باتشولي", "مسك أبيض"],
    img: "candle9.jpg",
    imgs: ["candle9.jpg"],
    sold: 0,
    createdAt: NOW - 9 * D,
  ...
  },
  {
    ...
    scents: ["ورد", "ياسمين", "مسك أبيض", "فانيليا"],
    img: "box1.jpg",
    imgs: ["box1.jpg"],
    sold: 0,
    createdAt: NOW - 14 * D,
...
const LocalDB = {
  ...
  g(c) {
    try {
      return JSON.parse(localStorage.getItem(this.k(c))) || [];
    } catch (e) {
      return [];
    }
  },
  s(c, v) {
    localStorage.setItem(this.k(c), JSON.stringify(v));
  },
  async list(c) {
    return this.g(c);
  },
  async add(c, d) {
    const a = this.g(c);
    d.id = "l" + Date.now() + Math.random().toString(16).slice(2, 7);
    a.push(d);
    this.s(c, a);
    return d.id;
  },
  async update(c, id, d) {
    this.s(
      c,
      this.g(c).map((x) => (x.id === id ? { ...x, ...d } : x))
    );
  },
  async remove(c, id) {
    this.s(
      c,
      this.g(c).filter((x) => x.id !== id)
    );
  }
};
...
async function loadAll() {
  ...
  ALL_PRODUCTS = Array.from(map.values()).filter(
    (p) => p && p.id && (p.name || p.nameEn)
  );
  ...
  try {
    ...
    console.error("Firestore reviews read error:", e);
  }
...
const prodReviews = (s) =>
  ALL_REVIEWS.filter(
    (r) => String(r.productSlug || r.pid || "") === String(s) && r.approved !== false
  );

const ratingOf = (s) => {
  const r = prodReviews(s);
  if (!r.length) return null;
  return {
    avg: +(r.reduce((a, b) => a + (+b.rating || 5), 0) / r.length).toFixed(1),
    count: r.length
  };
};
...
const I18N = {
  ...
  en: {
    ...
    hero_b: "and a Sparkle That Suits You",
    ...
    foot_shop: "Shop",
    foot_admin: "Admin",
    foot_pay_h: "Payment and Shipping",
    pay_title: "InstaPay",
    foot_pay_note: "InstaPay upfront, shipping cash.",
    ...
    paymethod_d: "InstaPay upfront, shipping cash.",
    checkout_wa: "✅ Order via WhatsApp",
    empty_cart: "🗑️ Empty",
    cart_empty: "Cart empty 🕯️",
...
const saveCart = (c) => {
  localStorage.setItem("vl_cart", JSON.stringify(c));
  cartBadge();
};

function addToCart(p, opt = {}) {
  const c = getCart();
  const scent = opt.scent || ((p.scents || [])[0] || "—");
